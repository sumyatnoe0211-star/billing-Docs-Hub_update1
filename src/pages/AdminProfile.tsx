...
export default function AdminProfile() {
  ...
  const [user, setUser] = useState<UserProfile | null>(null); // Local state for profile
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      try {
        ...
        console.error("AdminProfile fetch error:", err);
  ...
  const handleUpdate = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setUser(updatedUser);
    localStorage.setItem(STORAGE_KEY, JSON.stringify(updatedUser)); // Update local storage
  ...
  return (
    <div style={{ padding: '1.5rem', maxWidth: '400px', margin: 'auto', backgroundColor: colors.white, borderRadius: '0.5rem', boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }}>
      <h2 style={{ fontSize: typography.fontSizes.xLarge, fontWeight: typography.fontWeights.bold, textAlign: 'center', marginBottom: '1rem', color: colors.primary }}>Admin Profile</h2>
